...
6. Push commits and tags using the bot identity wrapper supplied by
   ...
   repository files.
7. Complete the source release after its own tag, checks and GitHub release are verified.
   Atlas observes the published source asynchronously. Report documentation as pending until
   the governed Website publication and canonical route are verified; a local build alone is
